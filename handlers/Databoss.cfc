...
component extends="BaseHandler"{
	...
	function index( event, rc, prc ){
		...
		prc.export = false;
		switch( rc.format ){
			case "json" : case "xml" : case "wddx" : case "jsonp" : {
				event.renderData(
					type		= rc.format,
    ...
    function doDelete(event, rc, prc){
		...
		if ( len( event.getTrimValue( "entityID", "" ) ) ){
			...
			rc.entityID.listToArray()
				.each( function( item ){
					try{
						entityService.delete( 
							entityService.get( entityName=rc.entity, id=item )
						);
					} catch( any e ){
			...
			if( arrayLen( errors ) eq 0 ){
				flash.put( "notice", { type="success", message=getResource('db_record_deletion_complete@db') } );
			} else {
			...
			// Message
			flash.put( "notice", { type="warning", message=getResource( 'db_record_deletion_no_records@db' ) } );
		}
    ...
    function create(event, rc, prc){
		...
		if ( prc.mdDictionary.hasSingularRelations ){
			...
			for( var thisRelation in prc.mdDictionary.singularRelations ){
				...
				// get a listing
				prc[ thisRelation.alias ] = entityService.list( entityName=thisRelation.sourceEntity, sortOrder=thisRelationMD.sortBy, asQuery=false );
			}
		}
		//Set view.
    ...
    function doCreate(event, rc, prc){
		...
		if( !coldboxValidation( oEntity, "create", arguments.event, arguments.rc, arguments.prc ) ){ 
			return; 
		}
		...
		for (var i=1; i lte ArrayLen( prc.mdDictionary.singularRelations ); i++ ){
			...
			if(
				...
				invoke( oEntity, "set#prc.mdDictionary.singularRelations[ i ].alias#", [ tmpEntity ] );
			}
		}
    ...
    function editor(event, rc, prc){
		...
		if( prc.mdDictionary.hasCollections ) {
			...
			for (var i=1; i lte ArrayLen( prc.mdDictionary.collections ); i++){
				...
				prc[ "#tmpAlias#Dictionary" ] = metadataService.getDictionaryEntry( prc.mdDictionary.collections[ i ].sourceEntity );
				// store lookup queries for additions
				prc[ "#tmpAlias#" ] = entityService.list( entityName=prc.mdDictionary.collections[ i ].sourceEntity, sortOrder=prc[ "#tmpAlias#Dictionary" ].sortBy, asQuery=false );
    ...
    function doUpdate( event, rc, prc ){
		...
		// Validate entity if possible via ColdBox validation
		if( !coldboxValidation( oEntity, "update", arguments.event, arguments.rc, arguments.prc ) ){ return; }
		//Populate it with RC data
		populateModel( oEntity );
		//Loop Through relations
		for ( var i=1;i lte arrayLen( prc.mdDictionary.singularRelations ); i++ ){
			// verify if fk exists in incoming rc
			if( structKeyExists( rc, "fk_" & prc.mdDictionary.singularRelations[ i ].alias )
				AND  len( rc["fk_" & prc.mdDictionary.singularRelations[ i ].alias ] ) GT 0 ){
				// Is this null?
    ...
    function doUpdateRelation(event, rc, prc){
		...
		if ( rc.addrelation ){
			...
			if ( NOT invoke( oEntity, "has#relationSingularName#", [ oRelation ] ) ){
				//Add Relation to parent
				invoke( oEntity, "add#relationSingularName#", [ oRelation ] );
			}
		}
			...
			for (var i=1; i lte listlen( deleteRelationList ); i++){
				...
				invoke( oEntity, "remove#relationSingularName#", [ oRelation ] );
			}
		}
    ...
    private function queryToStruct( required target ){
		listToArray( arguments.target.columnlist )
